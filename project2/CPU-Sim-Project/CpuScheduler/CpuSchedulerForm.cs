...
4. Compare algorithm behaviors and outcomes";
        ...
        private List<SchedulingResult> RunSRTFAlgorithm(List<ProcessData> processes)
        {
            ...
            // Calculate a safer maximum time
            int totalBurstTime = processes.Sum(p => p.BurstTime);
            int maxArrivalTime = processes.Max(p => p.ArrivalTime);
            int maxFinishTime = maxArrivalTime + totalBurstTime + 100;

            while (completed < processes.Count && currentTime < maxFinishTime)
            {
            ...
            // Create results only for completed processes
            foreach (var process in processes)
            {
                // Check if process was completed
                if (!completionTimes.ContainsKey(process.ProcessID))
                {
                    // Process didn't complete - this shouldn't happen with correct logic
                    MessageBox.Show(
                        $"Warning: Process {process.ProcessID} did not complete. " +
                        "Check arrival times and burst times.",
                ...
                var finishTime = completionTimes[process.ProcessID];
                var turnaroundTime = finishTime - process.ArrivalTime;
        ...
        /// Highest Response Ratio Next - selects based on response ratio
        /// </summary>
        private List<SchedulingResult> RunHRRNAlgorithm(List<ProcessData> processes)
        {
            ...
            while (remainingProcesses.Count > 0)
            {
                ...
                foreach (var process in availableProcesses)
                {
                    ...
                    var responseRatio = (waitingTime + process.BurstTime) / (double)process.BurstTime;
                ...
                var startTime = Math.Max(currentTime, selectedProcess.ArrivalTime);
                var finishTime = startTime + selectedProcess.BurstTime;
                var waitTime = startTime - selectedProcess.ArrivalTime;
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            var avgResponseTime = results.Average(r => r.StartTime - r.ArrivalTime);
            ...
            var cpuUtilization = totalTime > 0 ? (totalBurstTime / (double)totalTime) * 100 : 0;
            var throughput = totalTime > 0 ? results.Count / (double)totalTime : 0;

            // Add separator line
            var separatorItem = new ListViewItem("───────────");
            for (int i = 0; i < 6; i++)
                separatorItem.SubItems.Add("───────────");
            listView1.Items.Add(separatorItem);

            // Add summary statistics
            var summaryItem = new ListViewItem("SUMMARY");
            summaryItem.SubItems.Add(algorithmName);
            summaryItem.SubItems.Add($"{results.Count} proc");
            ...
            awtItem.SubItems.Add($"{avgWaiting:F2} units");
            awtItem.SubItems.Add("");
            awtItem.SubItems.Add("");
            awtItem.SubItems.Add("");
            awtItem.SubItems.Add("");
            awtItem.SubItems.Add("");
            ...
            attItem.SubItems.Add($"{avgTurnaround:F2} units");
            attItem.SubItems.Add("");
            attItem.SubItems.Add("");
            attItem.SubItems.Add("");
            attItem.SubItems.Add("");
            attItem.SubItems.Add("");
            ...
            artItem.SubItems.Add($"{avgResponseTime:F2} units");
            artItem.SubItems.Add("");
            artItem.SubItems.Add("");
            artItem.SubItems.Add("");
            artItem.SubItems.Add("");
            artItem.SubItems.Add("");
            ...
            var cpuItem = new ListViewItem("CPU Utilization");
            cpuItem.SubItems.Add($"{cpuUtilization:F2}%");
            cpuItem.SubItems.Add("");
            cpuItem.SubItems.Add("");
            cpuItem.SubItems.Add("");
            cpuItem.SubItems.Add("");
            cpuItem.SubItems.Add("");
            ...
            throughputItem.SubItems.Add($"{throughput:F4} proc/unit");
            throughputItem.SubItems.Add("");
            throughputItem.SubItems.Add("");
            throughputItem.SubItems.Add("");
            throughputItem.SubItems.Add("");
            throughputItem.SubItems.Add("");
            ...
            var totalTimeItem = new ListViewItem("Total Time");
            totalTimeItem.SubItems.Add($"{totalTime} units");
            totalTimeItem.SubItems.Add("");
            totalTimeItem.SubItems.Add("");
            totalTimeItem.SubItems.Add("");
            totalTimeItem.SubItems.Add("");
            totalTimeItem.SubItems.Add("");
            listView1.Items.Add(totalTimeItem);
        ...
        // Add these fields at the class level (near the top with other fields)
        private List<SchedulingResult> lastResults = new List<SchedulingResult>();
        private string lastAlgorithmName = "";
        private PerformanceMetrics lastMetrics = null;

        /// <summary>
        /// STUDENTS: Data structure for performance metrics
        /// </summary>
        ...
        /// <summary>
        /// STUDENTS: Exports scheduling results and performance metrics to CSV file
        /// This helps create tables and charts for your project report
        /// </summary>
        private void ExportResults_Click(object sender, EventArgs e)
        {
            if (lastResults == null || lastResults.Count == 0)
            {
                MessageBox.Show("No results to export. Please run a scheduling algorithm first.",
            ...
            using (var saveDialog = new SaveFileDialog())
            {
                ...
                saveDialog.FileName = $"{lastAlgorithmName.Replace(" ", "_")}_Results.csv";
                ...
                if (saveDialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        using (var writer = new System.IO.StreamWriter(saveDialog.FileName))
                        {
                            ...
                            writer.WriteLine($"Number of Processes,{lastResults.Count}");
                            writer.WriteLine($"Export Date,{DateTime.Now:yyyy-MM-dd HH:mm:ss}");
                            writer.WriteLine();

                            // Write process results header
                            writer.WriteLine("PROCESS RESULTS");
                            writer.WriteLine("Process ID,Arrival Time,Burst Time,Start Time,Finish Time,Waiting Time,Turnaround Time,Response Time");

                            // Write process data
                            foreach (var result in lastResults)
                            {
                                var responseTime = result.StartTime - result.ArrivalTime;
                                writer.WriteLine($"{result.ProcessID},{result.ArrivalTime},{result.BurstTime},{result.StartTime},{result.FinishTime},{result.WaitingTime},{result.TurnaroundTime},{responseTime}");
                            }
                            ...
                            writer.WriteLine($"Average Waiting Time,{lastMetrics.AverageWaitingTime:F2},time units");
                            writer.WriteLine($"Average Turnaround Time,{lastMetrics.AverageTurnaroundTime:F2},time units");
                            writer.WriteLine($"Average Response Time,{lastMetrics.AverageResponseTime:F2},time units");
                            ...
                            writer.WriteLine($"Total Execution Time,{lastMetrics.TotalTime},time units");

                            writer.WriteLine();

                            // Write summary statistics
                            writer.WriteLine("SUMMARY STATISTICS");
                            writer.WriteLine($"Total Waiting Time,{lastResults.Sum(r => r.WaitingTime)}");
                            writer.WriteLine($"Total Turnaround Time,{lastResults.Sum(r => r.TurnaroundTime)}");
                            writer.WriteLine($"Total Burst Time,{lastResults.Sum(r => r.BurstTime)}");
                            writer.WriteLine($"Min Waiting Time,{lastResults.Min(r => r.WaitingTime)}");
                            writer.WriteLine($"Max Waiting Time,{lastResults.Max(r => r.WaitingTime)}");
                            writer.WriteLine($"Min Turnaround Time,{lastResults.Min(r => r.TurnaroundTime)}");
                            writer.WriteLine($"Max Turnaround Time,{lastResults.Max(r => r.TurnaroundTime)}");
                        }

                        MessageBox.Show($"Results exported successfully to:\n{saveDialog.FileName}\n\nYou can now open this file in Excel to create charts and tables for your report.",
        ...
        /// STUDENTS: NEW algorithm - Shortest Remaining Time First (preemptive SJF)
        /// </summary>
        ...
        /// <summary>
        /// Executes the HRRN algorithm using DataGrid data.
        /// STUDENTS: NEW algorithm - Highest Response Ratio Next
        /// </summary>
        ...
        private void CpuSchedulerForm_Load(object sender, EventArgs e)
        {
            ...
            btnExportResults.Text = "📊 Export Results to CSV";
            btnExportResults.Size = new Size(200, 40);
            btnExportResults.Location = new Point(20, 20); // Adjust position as needed
            btnExportResults.Click += ExportResults_Click;
        ...
        private void ApplyLightTheme()
        {
            ...
            btnHRRN.BackColor = Color.LightGoldenrodYellow;
